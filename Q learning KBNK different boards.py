...
import gym_chess
import random
from helpers import custom_reset, custom_step, find_max_Q_move_KBNK, pos_to_index_KBNK
import numpy as np
import copy
import pandas as pd
...
Q_table = np.append(Q_table_white, Q_table_black, axis=4)
...
squares_small_board = [i for i in range(squares)]
#squares_small_board = [33,34,35,36,40,41,42,43,48,49,50,51,56,57,58,59]
#squares_small_board = [i for i in range(64)]
#chess_boards = [[33,34,35,36,40,41,42,43,48,49,50,51,56,57,58,59],[24,25,26,27,28,32,33,34,35,36,40,41,42,43,44,48,49,50,51,52,56,57,58,59,60],
                # [16,17,18,19,20,21, 24,25,26,27,28,29, 32,33,34,35,36,37, 40,41,42,43,44,55, 48,49,50,51,52,53, 56,57,58,59,60,61],
...
aggr_results = {'ep': [i for i in range(0, episodes+Show_Every, Show_Every)] , 'avg':[]}
...
for episode in tqdm(range(episodes+1)):
    ...
    Fen = generate_endgame_FEN(squares_small_board, [chess.BISHOP, chess.KNIGHT])
    ...
    while not done and len(played_moves) < 100: # with 50 move rule
        ...
        if obs.turn: # if White to move
            ...
            Q_value_best_move_t1, best_move_t1, a_t1, b_t1 = find_max_Q_move_KBNK(moves, Q_table, obs, K_t1, B_t1, N_t1, k_t1) # wel of geen t_1 erbij?????
            ...
            Q_value_best_move_t1, best_move_t1, a_t1, a_t2 = find_max_Q_move_KBNK(moves, Q_table, obs, K_t1, B_t1, N_t1, k_t1)
            obs_t1, reward_t1, done_t1, info_t1 = new_env.step(new_env, best_move_t1)
...
plt.plot(aggr_results['ep'], aggr_results['avg'], label='avg win rate 5x5')
